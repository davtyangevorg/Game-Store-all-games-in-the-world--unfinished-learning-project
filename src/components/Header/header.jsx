...
const Header = () => {
    ...
    useEffect(() => {
        ...
        if (!inputCurent.current.value) setIsShowSearchResults(false)
    }, [searchResults])
        ...
        if (index <= 5 && el.parent_platforms) {
            return <div className={style.game} key={el.id}>
                <div style={{ width: '40px', height: '50px' }}>
                    <img style={{ width: '100%', height: '50px', objectFit: 'cover', borderRadius: '5px' }}
                        alt='n' src={el.background_image}>
                    </img>
                </div>
                <div className={style.game_text}>
                    <div style={{ display: 'flex', marginBottom: '5px' }}>{getParentPlatformInons(el.parent_platforms)}</div>
                    <div>
                        <Link onClick={() => { setIsShowSearchResults(false); inputCurent.current.value = ''; setIsFocus(false) }}
                            className={style.game_text_link}
                            to={`/home/games/${el.slug}`}>{el.name}
                        </Link>
                    </div>
                </div>
            </div>
        ...
        <div className={style.header}>
            ...
            <div onBlur={() => { setIsFocus(false) }}
                ...
                <input className={style.input}
                    ...
                    onClick={() => { searchResults.length !== 0 && setIsShowSearchResults(true) }}
                    ref={inputCurent} role="searchbox"
